...
df2 = pd.read_excel(r'C:\Users\Renan\Desktop\PyProjs\Dsic Tracker\main table plus socials plus genre plus chains.xls')
...
for id__ in id_:
    ...
    sheet1.write(i+1, 20, blockchains[i])
    ...
    if(not str(mylinks) == '[]'):    
        ...
        for block in devices:
            if('title=' in block):
                ...
                devices_instance = devices_instance.replace('title="','')
                devices_instance = devices_instance.replace('"><div','')
...
wb.save('main table plus socials plus genre plus chains plus devices.xls')
# run test first!!
